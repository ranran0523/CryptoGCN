...
vector<vector<vector<vector<double>>>> stgcn(int input_channel, int output_channel, vector<vector<vector<vector<double>>>> X, vector<vector<vector<double>>> A, vector<vector<vector<double>>> importance, vector<vector<vector<double>>> w1, vector<vector<vector<double>>> w2, vector<double>b1, vector<double>b2, int stride, int residual, vector<double> mean1, vector<double> mean2, vector<double> var1, vector<double> var2, vector<double> gamma1, vector<double> gamma2, vector<double> beta1, vector<double> beta2, vector<vector<double>> w_res, vector<double> b_res, vector<double> mean_res, vector<double> var_res, vector<double> gamma_res, vector<double> beta_res)
{
    // first 1 by 1 conv kernel computation in spatial domain
    vector<vector<vector<vector<vector<double>>>>> conv11(X.size(),vector<vector<vector<vector<double>>>>(A.size(), vector<vector<vector<double>>>(output_channel, vector<vector<double>>(X[0][0].size(), vector<double>(X[0][0][0].size(),0)))));
    for (int m=0; m<X.size(); m++){                             // m - dimension for persons 
		for (int p=0; p<A.size();p++){							// p - dimension for the paritions
			for (int o=0; o<output_channel;o++){    			// o - dimension for the output channel
				for (int j=0; j<X[0][0].size();j++){			// j - dimension for the frames
					for (int k=0; k<X[0][0][0].size();k++){	    // k - dimension for the nodes				
						for (int i=0; i< X[0].size();i++){		// i - dimension for the input channel
							conv11[m][p][o][j][k] += X[m][i][j][k]*w1[p][o][i];
						}
						conv11[m][p][o][j][k] += b1[p*output_channel+o];
					}
				}
			}
		}
    ...
    // after the first spatil conv computation, times with normalized A matrix
    vector<vector<vector<vector<double>>>> graphconv(X.size(), vector<vector<vector<double>>>(output_channel, vector<vector<double>>(X[0][0].size(), vector<double>(X[0][0][0].size(), 0))));
	for (int m=0; m<X.size(); m++){                             // m - dimension for persons 
		for (int o=0; o<output_channel;o++){                    // o - dimension for the output channel
			for (int j=0; j<conv11[0][0].size();j++){           // j - dimension for the frames
				for (int w=0; w<A[0][0].size();w++){            // w - dimension for the nodes (repeat)
					for (int p=0; p<A.size();p++){              // p - dimension for the paritions
						for(int k=0; k<A[0].size();k++){        // k - dimension for the nodes 
							graphconv[m][o][j][w] += conv11[m][p][o][j][k]*(A[p][k][w]*importance[p][k][w]);
						}
					}
				}
			}
	...
	// the first bn layer with relu activation in tcn module
	vector<vector<vector<vector<double>>>> bn1(X.size(),vector<vector<vector<double>>>(output_channel, vector<vector<double>>(X[0][0].size(), vector<double>(X[0][0][0].size(), 0))));
	for (int m=0; m<X.size(); m++){                             // m - dimension for persons 
		for (int o=0; o<output_channel;o++){                    // o - dimension for the output channel
			for (int j=0; j<bn1[0][0].size();j++){              // j - dimension for the frames
				for (int w=0; w<A[0][0].size();w++){            // w - dimension for the nodes (repeat)
					
					bn1[m][o][j][w] = ((graphconv[m][o][j][w] - mean1[o])/var1[o] ) * gamma1[o]+ beta1[o];                // need configuration confirmation for the BN layer
					
					if (bn1[m][o][j][w] >= 0){
						bn1[m][o][j][w]=bn1[m][o][j][w];              
					}else{
						bn1[m][o][j][w]=0;
					}					
				}
	...
	//padding for second 9 by 1 kernel conv 
	int t_kernel=w2[0][0].size();

	vector<vector<vector<vector<double>>>> bn1_padding(X.size(),vector<vector<vector<double>>>(output_channel, vector<vector<double>>((X[0][0].size()+t_kernel-1), vector<double>(X[0][0][0].size(), 0))));
	for (int m=0; m<X.size(); m++){                             		// m - dimension for persons 
		for (int o=0; o<output_channel;o++){                    		// o - dimension for the output channel
			for (int j=0; j<bn1[0][0].size();j++){              		// j - dimension for the frames
				for (int w=0; w<bn1[0][0][0].size();w++){            	// w - dimension for the nodes (repeat)
					bn1_padding[m][o][j+(t_kernel-1)/2][w]=bn1[m][o][j][w];
				}
			}
		}
	}
	...
	//residual design
	vector<vector<vector<vector<double>>>> res(X.size(),vector<vector<vector<double>>>(output_channel, vector<vector<double>>((X[0][0].size()/stride), vector<double>(X[0][0][0].size(), 0))));
	vector<vector<vector<vector<double>>>> conv31(X.size(),vector<vector<vector<double>>>(output_channel, vector<vector<double>>((X[0][0].size()/stride), vector<double>(X[0][0][0].size(), 0))));
	...
	if( stride == 1 ){
		...
		for (int m=0; m<X.size(); m++){                                                 // m - dimension for persons 
			for (int o=0; o< output_channel; o++){                                      // o - dimension for the output channel
				for (int j=0; j<conv31[0][0].size() ; j++){			                    // j - dimension for the frames
					for (int w=0; w<conv31[0][0][0].size(); j++){                       // w - dimension for the nodes				
						for (int i=0; i<output_channel;i++){				            // i - dimension for the input channel					
							for (int t=0; t< w2[0][0].size();t++){						// t - dimension for the temporal kernel size							
								conv31[m][o][j][w] += bn1_padding[m][i][j+t][w]*w2[o][i][t];
							}
						}
						conv31[m][o][j][w] += b2[o];
					}
		...
		//res conv layer
		//vector<vector<vector<vector<double>>>> res_conv(X.size(),vector<vector<vector<double>>>(output_channel, vector<vector<double>>( (X[0][0].size()/stride), vector<double>(X[0][0][0].size(), 0))));
		//vector<vector<vector<vector<double>>>> conv31(X.size(),vector<vector<vector<double>>>(output_channel, vector<vector<double>>((X[0][0].size()/stride), vector<double>(X[0][0][0].size(), 0))));


		for (int m=0; m<X.size(); m++){                                                 // m - dimension for persons 
			for (int o=0; o< output_channel; o++){                                      // o - dimension for the output channel
				for (int j=0; j<res[0][0].size() ; j++){			                    // j - dimension for the frames
					for (int w=0; w<res[0][0][0].size(); j++){                          // w - dimension for the nodes
						for (int i=0; i<input_channel;i++){				                // i - dimension for the input channel			
							res[m][o][j][w] += X[m][i][2*j][w]* w_res[o][i];
						}
						res[m][o][j][w] += b_res[o];
					}
				}
		...
		for (int m=0; m<X.size(); m++){                                       // m - dimension for persons 
			for (int o=0; o<output_channel;o++){                              // o - dimension for the output channel
				for (int j=0; j<res[0][0].size();j++){                        // j - dimension for the frames
					for (int w=0; w<res[0][0][0].size();w++){                 // w - dimension for the nodes (repeat)
						
						res[m][o][j][w] = ((res[m][o][j][w] - mean_res[j]) / var_res[j] ) * gamma_res[j]+ beta_res[j];         // need configuration confirmation for the BN layer
					}
				}
			}
		}
		...
		for (int m=0; m<X.size(); m++){                                                 // m - dimension for persons 
			for (int o=0; o< output_channel; o++){                                      // o - dimension for the output channel
				for (int j=0; j< (conv31[0][0].size()/2) ; j++){			            // j - dimension for the frames
					for (int w=0; w<conv31[0][0][0].size(); j++){                       // w - dimension for the nodes					
						for (int i=0; i<output_channel;i++){				            // i - dimension for the input channel							
							for (int t=0; t< w2[0][0].size();t++){						// t - dimension for the temporal kernel size	
								conv31[m][o][j][w] += bn1_padding[m][i][2*j+t][w]*w2[o][i][t];
							}
						}
						conv31[m][o][j][w] += b2[o];
					}
	...
	// the second bn layer in tcn module
	vector<vector<vector<vector<double>>>> bn2(X.size(),vector<vector<vector<double>>>(output_channel, vector<vector<double>>(X[0].size(), vector<double>(X[0][0].size(), 0))));
	for (int m=0; m<X.size(); m++){                             // m - dimension for persons 
		for (int o=0; o<output_channel;o++){                    // o - dimension for the output channel
			for (int j=0; j<bn2[0][0].size();j++){              // j - dimension for the frames
				for (int w=0; w<bn2[0][0][0].size();w++){       // w - dimension for the nodes
					
					bn2[m][o][j][w] = ((conv31[m][o][j][w] - mean2[o]) / var2[o] ) * gamma2[o]+ beta2[o];         // need configuration confirmation for the BN layer
	...
	vector<vector<vector<vector<double>>>> result(X.size(),vector<vector<vector<double>>>(output_channel, vector<vector<double>>((X[0][0].size()/stride), vector<double>(X[0][0][0].size(), 0))));
	for (int m=0; m<X.size(); m++){                             // m - dimension for persons 
		for (int o=0; o<output_channel;o++){                    // o - dimension for the output channel
			for (int j=0; j<result[0][0].size();j++){           // j - dimension for the frames
				for (int w=0; w<result[0][0][0].size();w++){            // w - dimension for the nodes (repeat)
					
					result[m][o][j][w] = bn2[m][o][j][w] + res[m][o][j][w];         // need configuration confirmation for the BN layer

					if (result[m][o][j][w] >= 0){
						result[m][o][j][w]=result[m][o][j][w];            
					}else{
						result[m][o][j][w]=0;
					}	
...
void bootcamp_demo()
{
	...
	fc=read_params_2d("./params/fcn.weight.txt");

	vector<double>fc_bias;
	fc_bias=read_params_1d("./params/fcn.bias.txt");
	...
	// ofstream outputfile;
    // outputfile.open ("conv_result.txt");

    // for (int p=0; p<conv_result.size();p++){							//dimension for the paritions
    // 	for (int o=0; o<conv_result[0].size();o++){    			//dimension for the output channel
    // 		for (int j=0; j<conv_result[0][0].size();j++){				//dimension for the frames
    // 			for (int k=0; k<conv_result[0][0][0].size();k++){

	// 				outputfile<<conv_result[p][o][j][k]<<", ";
	...
	comment the encryption computation first, later uncomment this...
